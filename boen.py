    ...
    for pic in files:
        ...
        if diffRes > 0.95:
            if BoenSettings['dumping'] == 0:
                upload_url = vk.photos.getMessagesUploadServer(peer_id=peer_id)[
                    'upload_url']
                response = vk._vk.http.post(
                    upload_url, files={'photo': open(f'{cache_path}/{photo}', 'rb')})
                attcm = vk.photos.saveMessagesPhoto(**response.json())
                send_message(peer_id, f'{random.choice(BoenSettings["boenMessage"])} @id{from_id}',
                             event.obj['id'], f'photo{attcm[0]["owner_id"]}_{attcm[0]["id"]}')
...
def walker(cache_path, photo):
    ...
    if(len(files) > 2000):
        try:
            os.remove(
                str(sorted(Path(cache_path).iterdir(), key=os.path.getmtime)[0]))
        except IsADirectoryError:
            os.remove(
                str(sorted(Path(cache_path).iterdir(), key=os.path.getmtime)[1]))
...
def get_photo(raw_photo):
    ...
    if f'{photo[:-4]}diff.npz' in os.listdir(path=cache_path):
        ...
        for member in vk.messages.getConversationMembers(peer_id=peer_id)['items']:
            if len(event.obj['fwd_messages']) != 0 and event.obj['fwd_messages'][0]['from_id'] == member['member_id']:
                fwdflag = False
            else:
                continue
        if not fwdflag:
        ...
        if diffRes > 0.95:
            if BoenSettings['dumping'] == 0:
                ...
                response = vk._vk.http.post(
                    upload_url, files={'photo': open(f'{cache_path}/{photo}', 'rb')})
                attcm = vk.photos.saveMessagesPhoto(**response.json())
                send_message(peer_id, f'{random.choice(BoenSettings["fwdMessage"])} @id{from_id}',
                             event.obj['id'], f'photo{attcm[0]["owner_id"]}_{attcm[0]["id"]}')
            os.remove(f'{cache_path}/{photo[:-4]}difftest.npz')
            print(f'Dublicate removed with SSIM: {diffRes} with {photo}')
        else:
            os.rename(f'{cache_path}/{photo[:-4]}difftest.npz',
                      f'{cache_path}/{photo[:-5]}{random.randint(0, 9)}diff.npz')
        return 0
    img_file = open(f'{cache_path}/{photo}', 'wb')
    img_file.write(img.content)
    img_file.close()
...
def search_image(image_url_to_find):
    ...
    if platform.system() == 'Windows':
        driver = webdriver.Firefox(
            executable_path=f'{os.getcwd()}/geckodriver.exe')  # для винды
    else:
        driver = webdriver.Firefox(
            executable_path=f'{os.getcwd()}/geckodriver', firefox_options=opts)  # для убунту
    ...
    try:
        search_button_element = driver.find_element_by_xpath(
            '/html/body/header/div/div[2]/div[1]/form/div[1]/span/span/div[2]/button')
        ...
        alt_res_element = driver.find_element_by_xpath(
            '//a[@class="Link Thumb Thumb_hover_fade Thumb_border Thumb_rounded Thumb_type_inline"]')
        ...
        try:
            ...
            try:
                similar_images = driver.find_elements_by_class_name(
                    'cbir-similar__thumb')
                random.choice(similar_images[:5]).click()

                res_element = driver.find_element_by_class_name(
                    'MMImage-Origin')
                res_img_url = res_element.get_attribute('src')

                art = requests.get(res_img_url, stream=True, headers={
                                   "User-Agent": "Mozilla/5.0 (Windows; U; Windows NT 5.1; en-US; rv:1.9.2.8) Gecko/20100722 Firefox/3.6.8 GTB7.1 (.NET CLR 3.5.30729)", "Referer": "http://example.com"}, timeout=5)
                ...
                upload_url = vk.photos.getMessagesUploadServer(peer_id=peer_id)[
                    'upload_url']
                response = vk._vk.http.post(
                    upload_url, files={'photo': open(f'{cache_path}/buf/random_art.png', 'rb')})
                attcm = vk.photos.saveMessagesPhoto(**response.json())
                find_message += f'\nНо вот что-то похожее\nНу или мб это: {alt_res_url}'
                print(find_message)
                send_message(
                    peer_id, find_message, event.obj['id'], f'photo{attcm[0]["owner_id"]}_{attcm[0]["id"]}')
            except:
                print('SEARCH FAILED')
                pass
    except NoSuchElementException:
        send_message(peer_id, 'Яндексы опять что-то намутили....',
                     event.obj['id'])
    driver.quit()
    ...
    try:
        for event in longPoll.listen():
            if event.type == VkBotEventType.MESSAGE_NEW:
                ...
                # peer_id беседы или ЛС, откуда пришло сообщение
                ...
                if message in ['боен молчи', 'боен молчать', ',jty vjkxb', ',jty vjkxfnm']:
                    if from_id != 155523158:
                        send_message(peer_id, random.choice(
                            BoenSettings['accessDen']))
                        continue
                ...
                if message == 'боен арт':
                    ...
                    if 'пиздец' in message:
                        psi = '2.0'
                    art = requests.get(f'https://thisanimedoesnotexist.ai/results/psi-{psi}/seed{random.randint(10000, 99999)}.png', stream=True, headers={
                                       "User-Agent": "Mozilla/5.0 (Windows; U; Windows NT 5.1; en-US; rv:1.9.2.8) Gecko/20100722 Firefox/3.6.8 GTB7.1 (.NET CLR 3.5.30729)", "Referer": "http://example.com"}, timeout=5)
                    if not os.path.exists(f'{cache_path}/buf'):
                        os.mkdir(f'{cache_path}/buf')
                    art_file = open(f'{cache_path}/buf/random_art.png', 'wb')
                    ...
                    attcm = vk.photos.saveMessagesPhoto(**response.json())
                    send_message(
                        peer_id, 'Во', event.obj['id'], f'photo{attcm[0]["owner_id"]}_{attcm[0]["id"]}')
                    continue
                        ...
                        if 'copy_history' in event.obj['attachments'][i]['wall']:
                            for j in range(len(event.obj['attachments'][i]['wall']['copy_history'][0]['attachments'])):
                                if event.obj['attachments'][i]['wall']['copy_history'][0]['attachments'][j]['type'] == 'photo':
                                    photo = event.obj['attachments'][i]['wall']['copy_history'][
                                        0]['attachments'][j]['photo']['sizes'][3]['url']
                                    print(
                                        f'\nGet picture: {photo}\nfrom {peer_id}')
                                    if message in ['поиск', 'сурс']:
                                        search_image(photo)
                ...
                for i in range(len(event.obj['fwd_messages'])):
                    fwdflag = True
                    for member in vk.messages.getConversationMembers(peer_id=peer_id)['items']:
                        if event.obj['fwd_messages'][0]['from_id'] == member['member_id']:
                            fwdflag = False
                        else:
                            continue
                    if not fwdflag:
                        ...
                        if event.obj['fwd_messages'][i]['attachments'][j]['type'] == 'photo':
                            ...
                            for k in range(len(event.obj['fwd_messages'][i]['attachments'][j]['wall']['attachments'])):
                                if event.obj['fwd_messages'][i]['attachments'][j]['wall']['attachments'][k]['type'] == 'photo':
                                    photo = event.obj['fwd_messages'][i]['attachments'][j][
                                        'wall']['attachments'][k]['photo']['sizes'][3]['url']
                                    print(
                                        f'\nGet picture: {photo}\nfrom {peer_id}')
                                    if message in ['поиск', 'сурс']:
                                        search_image(photo)
                ...
                if message == 'боен пока':
                    ...
                    send_message(peer_id, random.choice(
                        ['Это аккордеон', 'Не', 'Не не', 'Не не не', 'Да не', 'Обязательно', 'Я тута', 'Звал?', 'Кнешн', 'Нахуй']), reply, None)
