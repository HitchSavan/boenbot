...
def serverError(adminId):
	send_message(adminId, "Сервер упал", None)
	...
	for pic in files:
		...
		if diffRes > 0.9:
			if dumping == 0:
				...
				response = vk._vk.http.post(upload_url, files={'photo': open(cache_path+'/'+photo, 'rb')})
...
import atexit
atexit.register(serverError, 2000000002)